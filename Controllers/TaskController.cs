...
public class TaskController : Controller
{
    ...
    private ShiftTaskHst? hst;
    private ShiftTask? currentTask;
    ...
    public IActionResult ListTask(short shiftId, ShiftTaskEnum sortShift = ShiftTaskEnum.NumberAsc)
    {
        ...
        ViewData["taskName"] = "";
        IQueryable<ShiftTask> listShifts = listContext.ShiftTasks.Where(a => a.ShiftId == shiftId && a.Status 
            && (a.LastAction!.Value.AddHours(12) < DateTime.Now || a.LastAction == null));
        listShifts = sortShift switch
        {
            ShiftTaskEnum.StartTimeAsc => listShifts.AsEnumerable().OrderBy(s => s.StartTime.Hours >= 0 && (s.StartTime.Hours <= 7 && s.StartTime.Minutes <= 30) ? s.StartTime.Add(TimeSpan.FromHours(24)) : s.StartTime).AsQueryable(),
            ShiftTaskEnum.StartTimeDesc => listShifts.AsEnumerable().OrderByDescending(s => s.StartTime.Hours >= 0 && (s.StartTime.Hours <= 7 && s.StartTime.Minutes <= 30) ? s.StartTime.Add(TimeSpan.FromHours(24)) : s.StartTime).AsQueryable(),
            ShiftTaskEnum.FinishTimeAsc => listShifts.AsEnumerable().OrderBy(s => s.FinishTime.Hours >= 0 && (s.FinishTime.Hours <= 7 && s.FinishTime.Minutes <= 30) ? s.FinishTime.Add(TimeSpan.FromHours(24)) : s.FinishTime).AsQueryable(),
            ShiftTaskEnum.FinishTimeDesc => listShifts.AsEnumerable().OrderByDescending(s => s.FinishTime.Hours >= 0 && (s.FinishTime.Hours <= 7 && s.FinishTime.Minutes <= 30) ? s.FinishTime.Add(TimeSpan.FromHours(24)) : s.FinishTime).AsQueryable(),
        ...
        hst = listContext.ShiftTaskHsts.Include(u => u.ShiftTask).Include(u => u.User)
            .Where(u => u.ShiftTask.StartTime < DateTime.Now.TimeOfDay 
                    ...
                    && (u.ShiftTask.LastAction.Value.Date.AddHours(12) < DateTime.Now || u.ShiftTask.LastAction == null)
    ...
    {
        currentTask = listContext.ShiftTasks.Find(taskId);
        if (currentTask == null) 
            return RedirectToAction("ErrorIndex", "Home");
        if (currentTask.StartTime.Days >= 1)
            currentTask.StartTime = currentTask.StartTime.Add(new TimeSpan(-1, 0, 0, 0));
        if (currentTask.FinishTime.Days >= 1)
            currentTask.FinishTime = currentTask.FinishTime.Add(new TimeSpan(-1, 0, 0, 0));
        ViewBag.LastNumber = "Последний номер: " + listContext.ShiftTasks.Where(x => x.ShiftId == currentTask.ShiftId).Max(u => u.Number);
        ViewBag.Shifts = new SelectList(listContext.Shifts, "Id", "Name");
    ...
    public ActionResult DeleteTask(int taskId, string? tittle, short shiftId = 0, AdminShiftEnum pageSort = AdminShiftEnum.StartTimeAsc)
    {
        ...
        if (currentTask == null)
            return RedirectToAction("ErrorIndex", "Home");
        currentTask.Status = currentTask.Status ? false : true;
        listContext.Entry(currentTask).State = EntityState.Modified;
        listContext.SaveChanges();
    ...
    public ActionResult DeleteNotify(int taskId, string? tittle, short shiftId = 0, AdminShiftEnum pageSort = AdminShiftEnum.StartTimeAsc)
    {
        ...
        if (currentTask == null)
            return RedirectToAction("ErrorIndex", "Home");
        currentTask.NotifyUser = currentTask.NotifyUser ? false : true;
        listContext.Entry(currentTask).State = EntityState.Modified;
        listContext.SaveChanges();
        return RedirectToAction("AdminShift", new { tittle, shiftId, pageSort });
    }
